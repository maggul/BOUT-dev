...
if(BOUT_USE_HYPRE)
  ...
  if(HYPRE_WITH_CUDA AND BOUT_HAS_CUDA)
    target_compile_definitions(
      bout++ PUBLIC "HYPRE_USING_CUDA;HYPRE_USING_UNIFIED_MEMORY"
    )
    target_link_libraries(
      bout++ PUBLIC CUDA::cusparse CUDA::curand CUDA::culibos CUDA::cublas
                    CUDA::cublasLt
    )
  endif()
endif()
...
# PETSc
option(BOUT_USE_PETSC "Enable support for PETSc time solvers and inversions"
       OFF
)
if(BOUT_USE_PETSC)
...
cmake_dependent_option(
  BOUT_USE_SYSTEM_MPARK_VARIANT
  "Use external installation of mpark.variant"
  OFF
  "BOUT_UPDATE_GIT_SUBMODULE OR EXISTS ${PROJECT_SOURCE_DIR}/externalpackages/mpark.variant/CMakeLists.txt"
  ON
)
...
if(BOUT_USE_SYSTEM_MPARK_VARIANT)
  ...
  get_target_property(
    MPARK_VARIANT_INCLUDE_PATH mpark_variant INTERFACE_INCLUDE_DIRECTORIES
  )
else()
  ...
  if(NOT TARGET mpark_variant)
    message(
      FATAL_ERROR
        "mpark_variant not found! Have you disabled the git submodules (BOUT_UPDATE_GIT_SUBMODULE)?"
    )
  endif()
  set(MPARK_VARIANT_INCLUDE_PATH
      "${CMAKE_CURRENT_SOURCE_DIR}/externalpackages/mpark.variant/include"
  )
...
cmake_dependent_option(
  BOUT_USE_SYSTEM_FMT
  "Use external installation of fmt"
  OFF
  "BOUT_UPDATE_GIT_SUBMODULE OR EXISTS ${PROJECT_SOURCE_DIR}/externalpackages/fmt/CMakeLists.txt"
  ON
)
...
if(BOUT_USE_SYSTEM_FMT)
  ...
  if(NOT TARGET fmt::fmt)
    message(
      FATAL_ERROR
        "fmt not found! Have you disabled the git submodules (BOUT_UPDATE_GIT_SUBMODULE)?"
    )
  endif()
  # Build the library in <build dir>/lib: this makes updating the path
  # for bout-config much easier
  set_target_properties(
    fmt PROPERTIES LIBRARY_OUTPUT_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}/lib"
                   ARCHIVE_OUTPUT_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}/lib"
  )
  set(FMT_INCLUDE_PATH
      "${CMAKE_CURRENT_SOURCE_DIR}/externalpackages/fmt/include"
  )
...
if(BOUT_USE_NETCDF)
  if(BOUT_DOWNLOAD_NETCDF_CXX4)
    ...
    if(netCDFCxx_FOUND)
      set(CONFIG_LDFLAGS
          "${CONFIG_LDFLAGS} ${netCDF_CXX_LIBRARY} ${netCDF_LIBRARIES}"
      )
      target_link_libraries(bout++ PUBLIC netCDF::netcdf-cxx4)
    else()
  ...
  if(netCDFCxx_DIR)
    set(netCDFCxx_ROOT "${netCDFCxx_DIR}")
  endif()
endif()
...
if(BOUT_USE_SUNDIALS)
  ...
  if(BOUT_DOWNLOAD_SUNDIALS)
    ...
    set(EXAMPLES_ENABLE_C
        OFF
        CACHE BOOL "" FORCE
    )
    set(EXAMPLES_INSTALL
        OFF
        CACHE BOOL "" FORCE
    )
    set(ENABLE_MPI
    ...
    if(SUNDIALS_VERSION VERSION_LESS 4.0.0)
      message(
        FATAL_ERROR
          "SUNDIALS_VERSION 4.0.0 or newer is required. Found version ${SUNDIALS_VERSION}."
      )
    endif()
  endif()
  if(SUNDIALS_DIR)
  ...
  set(CONFIG_LDFLAGS
      "${CONFIG_LDFLAGS} ${SUNDIALS_cvode_LIBRARY} ${SUNDIALS_ida_LIBRARY} ${SUNDIALS_arkode_LIBRARY} ${SUNDIALS_nvecparallel_LIBRARY}"
  )
endif()
...
set(BOUT_USE_NLS
    AUTO
    CACHE STRING "Enable Native Language Support"
)
...
if(BOUT_USE_NLS)
  ...
  if(GETTEXT_FOUND)
    find_package(Intl)
    if(Intl_FOUND)
      target_link_libraries(bout++ PUBLIC ${Intl_LIBRARIES})
      target_include_directories(bout++ PUBLIC ${Intl_INCLUDE_DIRS})
...
if(BOUT_USE_SCOREP)
  message(
    STATUS
      "Score-P support enabled. Please make sure you are calling CMake like so:
...
option(BOUT_USE_UUID_SYSTEM_GENERATOR
       "Enable support for using a system UUID generator" ON
)
if(BOUT_USE_UUID_SYSTEM_GENERATOR)
  find_package(Libuuid QUIET)
  if(Libuuid_FOUND)
    target_link_libraries(bout++ PUBLIC Libuuid::libuuid)
...
cmake_dependent_option(
  BOUT_USE_SYSTEM_CPPTRACE
  "Use external installation of cpptrace"
  OFF
  "BOUT_UPDATE_GIT_SUBMODULE OR EXISTS ${PROJECT_SOURCE_DIR}/externalpackages/cpptrace/CMakeLists.txt"
  ON
)

if(BOUT_USE_SYSTEM_CPPTRACE)
  message(STATUS "Using external cpptrace")
  find_package(cpptrace REQUIRED)
  get_target_property(
    CPPTRACE_INCLUDE_PATH cpptrace::cpptrace INTERFACE_INCLUDE_DIRECTORIES
  )
else()
  message(STATUS "Using cpptrace submodule")
  bout_update_submodules()
  # Need a fork with some fixes for CMake
  set(CPPTRACE_LIBDWARF_REPO
      "https://github.com/ZedThree/libdwarf-lite.git"
      CACHE STRING "" FORCE
  )
  set(CPPTRACE_LIBDWARF_TAG
      "ebe10a39afd56b8247de633bfe17666ad50ab95e"
      CACHE STRING "" FORCE
  )
  add_subdirectory(externalpackages/cpptrace)
  if(NOT TARGET cpptrace::cpptrace)
    message(
      FATAL_ERROR
        "cpptrace not found! Have you disabled the git submodules (BOUT_UPDATE_GIT_SUBMODULE)?"
    )
  endif()
  set(CPPTRACE_INCLUDE_PATH
      "${CMAKE_CURRENT_SOURCE_DIR}/externalpackages/cpptrace/include"
  )
endif()
set(CONFIG_CFLAGS "${CONFIG_CFLAGS} -I\${CPPTRACE_INCLUDE_PATH}")
